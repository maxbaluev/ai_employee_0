...
import { MissionTemplateGroup, MissionTemplate } from "@/lib/types/mission";

type MissionLibraryPanelProps = {
  groups: MissionTemplateGroup[];
  onTemplateSelect?: (template: MissionTemplate) => void;
};

export function MissionLibraryPanel({ groups, onTemplateSelect }: MissionLibraryPanelProps) {
  return (
    <section
    ...
    >
      <header>
        ...
        <p className="text-sm text-slate-400">
          Persona-ready templates with time-to-value estimates and safeguard reminders.
        </p>
      </header>

      <div className="space-y-5">
        {groups.map((group) => (
          <article key={group.persona} className="space-y-3">
