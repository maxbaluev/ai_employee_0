...
- Narrates each mission stage (Define → Reflect & Improve) in real time.
...
| **3. Plan & Approve**    | Planner streams ranked plays (mission playbooks) based on tool usage patterns and data investigation insights, annotating sequencing, resource needs, and undo affordances before requesting approval. | Play cards with confidence badges, tool usage pattern highlights, safeguard recap, undo plans, scope alignment confirmations, "Request revision" buttons. | `planner_candidate_generated`, `plan_ranked`, `plan_approved` |
| **4. Execute & Observe** | Executor streams tool invocations line-by-line using established connections; validator injects alerts; evidence agent posts hashes. | Streaming log chip, validator alert threads, undo countdown message, artifact card with SHA badge. | `execution_started`, `composio_tool_call`, `validator_alert_raised`, `evidence_bundle_generated` |
| **5. Reflect & Improve** | Evidence agent summarises outcomes, prompts for feedback, suggests library reuse.                      | "Mission complete" summary, feedback form card, library suggestion chips, follow-up checklist.     | `feedback_submitted`, `mission_retrospective_logged`, `library_contribution`                     |
